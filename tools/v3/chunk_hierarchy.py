#!/usr/bin/env python3
"""
Break-point hierarchy chunker (candidate B, ACCEPTED 2026-07-11).

VERBATIM copy of technical/v3_chunking_test/chunk_hierarchy.py promoted
into tools/v3 for the production build (build3.py imports it).  Only
change: the sys.path line (tools/ is now parents[1], not parents[2]).
The experiment copy stays in place as the decision record; bug fixes
should land HERE and be back-ported there only if the experiment is
re-run.
...
TOOLS = Path(__file__).resolve().parents[1]  # tools/ (v3 copy: was parents[2])
